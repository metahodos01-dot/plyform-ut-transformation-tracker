...
export const NeedsAnalysis: React.FC = () => {
  ...
  const resetForm = () => {
    setEditingId(null);
    setForm({ description: '', reason: '', originator: '', date: new Date().toISOString().split('T')[0], priority: 'MEDIUM' });
    setAiResult(null);
  };
      ...
      <div className="bg-slate-900 rounded-2xl p-1 shadow-xl overflow-hidden flex-1 flex flex-col">
        <div className="bg-slate-900 p-6 sm:p-8 flex-1 flex flex-col">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1">
                {/* Form Column */}
                <div className={`lg:col-span-5 space-y-4 p-6 rounded-xl border transition-all h-fit ${
                  editingId ? 'bg-purple-900/20 border-purple-500/50' : 'bg-slate-800/50 border-slate-700'
                }`}>
                        ...
                        <div className="bg-slate-900 border border-purple-500/30 rounded-lg p-4 animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className={`text-xs font-bold px-2 py-0.5 rounded ${
                                    aiResult.impactLevel === 'HIGH' ? 'bg-emerald-500/20 text-emerald-400' :
                ...
                <div className="lg:col-span-7 bg-white rounded-xl overflow-hidden flex flex-col h-[calc(100vh-240px)] min-h-[600px]">
                    ...
                    <div className="overflow-y-auto p-4 space-y-3 flex-1">
                        ...
                        {needs.map((need, index) => {
                            ...
                            return (
                                <div key={need.id} className={`bg-white border rounded-lg p-0 hover:shadow-lg transition-all group flex flex-col sm:flex-row overflow-hidden ${
                                editingId === need.id ? 'border-purple-500 ring-1 ring-purple-500 bg-purple-50' : 'border-slate-200'
